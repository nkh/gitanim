...
# Tests that diffvim.pl can accept a .diff/.patch file or stdin as input.
...
my $out = `perl diffvim.pl --diff /tmp/dv_test_diff.patch --dry-run 2>&1`;
...
# Test 2: --diff - (stdin)
$out = `cat /tmp/dv_test_diff.patch | perl diffvim.pl --diff - --dry-run 2>&1`;
...
$out = `perl diffvim.pl --diff /tmp/dv_test_git.patch --dry-run 2>&1`;
ok('--diff with git-style a/b prefixes works', $out =~ /Dry run/ && $out =~ /Hunks: 1/);

# Test 4: --diff with invalid file
$out = `perl diffvim.pl --diff /nonexistent.patch --dry-run 2>&1`;
...
print $fh '';
close $fh;
$out = `perl diffvim.pl --diff /tmp/dv_empty.patch --dry-run 2>&1`;
