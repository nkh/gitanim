...
"$ROOT/bin/ad_layer_noop" < /tmp/pl_raw.txt > /tmp/pl_l0.txt 2>/dev/null
...
"$ROOT/bin/ad_layer_noop" < /tmp/pl_l0.txt > /tmp/pl_l1.txt 2>/dev/null
...
"$ROOT/bin/ad_layer_noop" < /tmp/pl_l1.txt > /tmp/pl_l2.txt 2>/dev/null
...
"$ROOT/bin/ad_layer_noop" < /tmp/pl_l2.txt > /tmp/pl_l3.txt 2>/dev/null
...
"$ROOT/bin/ad_layer_noop" < /tmp/pl_raw.txt 2>/dev/null | \
"$ROOT/bin/ad_layer_noop" 2>/dev/null | \
"$ROOT/bin/ad_layer_noop" 2>/dev/null | \
"$ROOT/bin/ad_layer_noop" > /tmp/pl_piped.txt 2>/dev/null
...
perl "$ROOT/animator/perl/ad_layer_noop.pl" < /tmp/pl_l0.txt > /tmp/pl_perl.txt 2>/dev/null
...
if grep -q "Layer 1" /tmp/ad_debug/postprocess.log 2>/dev/null && \
   grep -q "passthrough" /tmp/ad_debug/postprocess.log 2>/dev/null; then
...
"$ROOT/bin/ad_layer_pace" < /tmp/pl_piped.txt > /tmp/pl_timed.txt 2>/dev/null
"$ROOT/bin/ad" --no-display --speed 1000 --snapshot /tmp/pl_out.txt /tmp/pl_old.txt < /tmp/pl_timed.txt 2>/dev/null
