...
for my $mode (qw(none inline word hunk)) {
    my $out = `$DIFFVIM --highlight $mode --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
    ok("--highlight $mode accepted", $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --highlight invalid tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--highlight invalid rejected', $bad =~ /invalid --highlight value/);

# Test 4: Old flags are rejected (removed)
my $out_old_hw = `$DIFFVIM --highlight-word tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--highlight-word rejected', $out_old_hw =~ /Unknown option: --highlight-word/);

my $out_old_hh = `$DIFFVIM --highlight-hunk tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--highlight-hunk rejected', $out_old_hh =~ /Unknown option: --highlight-hunk/);

my $out_old_hi = `$DIFFVIM --highlight-inline tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--highlight-inline rejected', $out_old_hi =~ /Unknown option: --highlight-inline/);

# Test 5: Short options -W, -H, -I are rejected (removed)
my $out_short_w = `$DIFFVIM -W tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('-W rejected', $out_short_w =~ /Unknown option: -W/);

my $out_short_h = `$DIFFVIM -H tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('-H rejected', $out_short_h =~ /Unknown option: -H/);

my $out_short_i = `$DIFFVIM -I tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
...
my $out_combo = `$DIFFVIM --highlight inline --pacing gaussian --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--highlight combines with --pacing', $out_combo =~ /---/);

# Test 8: --highlight combines with --op-order and --delete-pacing
my $out_combo2 = `$DIFFVIM --highlight hunk --op-order end-first-smart --delete-pacing word --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
...
my $out_combo3 = `$DIFFVIM --highlight word --dim-unchanged --sign-column --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
