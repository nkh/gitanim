...
run_pipeline() {
    ...
    $ROOT/animator/bin/diffvim-postprocess $extra < "$TMPDIR/raw.txt" > "$TMPDIR/post.txt" 2>/dev/null
...
DELAY_UNI=$($ROOT/animator/bin/diffvim-pace --pacing uniform < "$TMPDIR/post.txt" 2>/dev/null | grep "^delay" | cut -f2 | sort -n | tail -1)
DELAY_REV=$($ROOT/animator/bin/diffvim-pace --pacing review < "$TMPDIR/post.txt" 2>/dev/null | grep "^delay" | cut -f2 | sort -n | tail -1)
ok "pacing review doubles delays" "[[ -n '$DELAY_UNI' ]] && [[ -n '$DELAY_REV' ]] && [[ $DELAY_REV -ge $DELAY_UNI ]]"

DELAY_GAUSS=$($ROOT/animator/bin/diffvim-pace --pacing gaussian < "$TMPDIR/post.txt" 2>/dev/null | grep "^delay" | grep -v "^delay      1       " | head -1 | cut -f2)
...
run_pipeline "$TMPDIR/old1.txt" "$TMPDIR/new1.txt"
BLOCK_START=$($ROOT/animator/bin/diffvim-pace --delete-pacing char --block-delete-size 1 --pause-before-delete-ms 100 < "$TMPDIR/post.txt" 2>/dev/null | grep "block_start" | wc -l)
ok "block-delete emits block_start pauses" "[[ $BLOCK_START -gt 0 ]]"

BLOCK_END=$($ROOT/animator/bin/diffvim-pace --delete-pacing char --block-delete-size 1 --pause-after-delete-ms 100 < "$TMPDIR/post.txt" 2>/dev/null | grep "block_end" | wc -l)
...
run_pipeline "$TMPDIR/old1.txt" "$TMPDIR/new1.txt"
LT_COUNT=$($ROOT/animator/bin/diffvim-postprocess --op-order left-to-right < "$TMPDIR/raw.txt" 2>/dev/null | wc -l)
