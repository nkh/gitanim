...
for my $n (0, 1, 3, 5, 10) {
    my $out = `$DIFFVIM --context $n --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--context $n accepted", $out =~ /---/);
}

# Test 3: -c short option works
my $out_short = `$DIFFVIM -c 3 --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('-c 3 works', $out_short =~ /---/);

# Test 4: --scroll accepts all 4 values
for my $mode (qw(zz zt zb none)) {
    my $out = `$DIFFVIM --scroll $mode --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--scroll $mode accepted", $out =~ /---/);
}

# Test 5: --fold-unchanged is rejected (removed, use --context 0)
my $out_fu = `$DIFFVIM --fold-unchanged tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--fold-unchanged rejected', $out_fu =~ /Unknown option: --fold-unchanged/);

# Test 6: -f short option is rejected (removed)
my $out_f = `$DIFFVIM -f tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('-f rejected', $out_f =~ /Unknown option: -f/);

# Test 7: --context 0 works (replaces --fold-unchanged)
my $out_c0 = `$DIFFVIM --context 0 --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
my $out_combo = `$DIFFVIM --context 3 --scroll zz --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--context combines with --scroll', $out_combo =~ /---/);

# Test 10: --context combines with all the new unified options
my $out_combo2 = `$DIFFVIM --context 3 --op-order end-first-smart --delete-pacing word --insert-pacing word --pacing review --highlight hunk --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
