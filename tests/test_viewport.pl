...
for my $n (0, 1, 3, 5, 10) {
    my $out = `$DIFFVIM --context $n --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
    ok("--context $n accepted", $out =~ /---/);
}

# Test 3: -c short option works
my $out_short = `$DIFFVIM -c 3 --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('-c 3 works', $out_short =~ /---/);

# Test 4: --scroll accepts all 4 values
for my $mode (qw(zz zt zb none)) {
    my $out = `$DIFFVIM --scroll $mode --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
...
ok('DIFFVIM_CONTEXT env var works', $out_env =~ /---/);
delete $ENV{DIFFVIM_CONTEXT};

# Test 9: --context combines with --scroll
my $out_combo = `$DIFFVIM --context 3 --scroll zz --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('--context combines with --scroll', $out_combo =~ /---/);

# Test 10: --context combines with all the new unified options
my $out_combo2 = `$DIFFVIM --context 3 --op-order end-first-smart --delete-pacing word --insert-pacing word --pacing review --highlight hunk --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
