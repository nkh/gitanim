...
for my $mode (qw(char rapid-eol rapid-identical accel word instant)) {
    my $out = `$DIFFVIM --delete-pacing $mode --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--delete-pacing $mode accepted", $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --delete-pacing invalid tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
for my $speed (qw(slow normal fast instant)) {
    my $out = `$DIFFVIM --delete-speed $speed --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--delete-speed $speed accepted", $out =~ /---/);
}

# Test 5: Invalid speed is rejected
my $bad_speed = `$DIFFVIM --delete-speed invalid tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--delete-speed invalid rejected', $bad_speed =~ /invalid --delete-speed value/);

# Test 6: --delete-threshold accepts a number
my $out_thr = `$DIFFVIM --delete-threshold 5 --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--delete-threshold accepted', $out_thr =~ /---/);

# Test 7: Old flags are rejected (removed)
my $out_old_reol = `$DIFFVIM --rapid-eol-delete tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--rapid-eol-delete rejected', $out_old_reol =~ /Unknown option: --rapid-eol-delete/);

my $out_old_accel = `$DIFFVIM --accel-delete tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--accel-delete rejected', $out_old_accel =~ /Unknown option: --accel-delete/);

my $out_old_awd = `$DIFFVIM --adaptive-word-delete tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--adaptive-word-delete rejected', $out_old_awd =~ /Unknown option: --adaptive-word-delete/);

my $out_old_ric = `$DIFFVIM --rapid-identical-chars tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
my $out_combo = `$DIFFVIM --delete-pacing word --op-order end-first-smart --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--delete-pacing combines with --op-order', $out_combo =~ /---/);

# Test 10: --delete-pacing instant + --delete-speed fast combine
my $out_combo2 = `$DIFFVIM --delete-pacing instant --delete-speed fast --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
