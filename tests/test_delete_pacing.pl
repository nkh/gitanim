...
for my $mode (qw(char rapid-eol rapid-identical accel word instant)) {
    my $out = `$DIFFVIM --delete-pacing $mode --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
    ok("--delete-pacing $mode accepted", $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --delete-pacing invalid examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
...
for my $speed (qw(slow normal fast instant)) {
    my $out = `$DIFFVIM --delete-speed $speed --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
    ok("--delete-speed $speed accepted", $out =~ /---/);
}

# Test 5: Invalid speed is rejected
my $bad_speed = `$DIFFVIM --delete-speed invalid examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
...
ok('DIFFVIM_DELETE_PACING env var works', $out_env =~ /---/);
delete $ENV{DIFFVIM_DELETE_PACING};

$ENV{DIFFVIM_DELETE_SPEED} = 'fast';
my $out_env_sp = `$DIFFVIM --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('DIFFVIM_DELETE_SPEED env var works', $out_env_sp =~ /---/);
delete $ENV{DIFFVIM_DELETE_SPEED};

# Test 9: --delete-pacing combines with --op-order
my $out_combo = `$DIFFVIM --delete-pacing word --op-order end-first-smart --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('--delete-pacing combines with --op-order', $out_combo =~ /---/);

# Test 10: --delete-pacing instant + --delete-speed fast combine
my $out_combo2 = `$DIFFVIM --delete-pacing instant --delete-speed fast --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
