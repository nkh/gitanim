...
for my $mode (qw(natural optimize left-to-right end-first end-first-smart overwrite)) {
    my $out = `$DIFFVIM --op-order $mode --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
    ok("--op-order $mode accepted", $out =~ /=== diffvim dry-run/ || $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --op-order invalid examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
...
my $out_natural = `$DIFFVIM --op-order natural --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('--op-order natural runs', $out_natural =~ /---/);

# Test 5: --op-order overwrite runs
my $out_overwrite = `$DIFFVIM --op-order overwrite --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
...
ok('DIFFVIM_OP_ORDER env var works', $out_env =~ /---/);
delete $ENV{DIFFVIM_OP_ORDER};

# Test 9: Short option -O still maps to --overwrite (not --op-order)
my $out_short = `$DIFFVIM -O --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('-O still maps to --overwrite', $out_short =~ /---/);

# Test 10: --op-order works with other options
my $out_combo = `$DIFFVIM --op-order left-to-right --word-diff --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
