...
for my $mode (qw(natural optimize left-to-right end-first end-first-smart overwrite)) {
    my $out = `$DIFFVIM --op-order $mode --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--op-order $mode accepted", $out =~ /=== diffvim dry-run/ || $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --op-order invalid tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
my $out_natural = `$DIFFVIM --op-order natural --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--op-order natural runs', $out_natural =~ /---/);

# Test 5: --op-order overwrite runs
my $out_overwrite = `$DIFFVIM --op-order overwrite --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--op-order overwrite runs', $out_overwrite =~ /---/);

# Test 6: --op-order end-first-smart runs
my $out_smart = `$DIFFVIM --op-order end-first-smart --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--op-order end-first-smart runs', $out_smart =~ /---/);

# Test 7: Old flags are rejected (removed)
my $out_old_opt = `$DIFFVIM --optimize-sequence tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--optimize-sequence rejected', $out_old_opt =~ /Unknown option: --optimize-sequence/);

my $out_old_l2r = `$DIFFVIM --left-to-right tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--left-to-right rejected', $out_old_l2r =~ /Unknown option: --left-to-right/);

my $out_old_def = `$DIFFVIM --delete-end-first tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--delete-end-first rejected', $out_old_def =~ /Unknown option: --delete-end-first/);

my $out_old_ovw = `$DIFFVIM --overwrite tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
my $out_short = `$DIFFVIM -O --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('-O still maps to --overwrite', $out_short =~ /---/);

# Test 10: --op-order works with other options
my $out_combo = `$DIFFVIM --op-order left-to-right --word-diff --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
