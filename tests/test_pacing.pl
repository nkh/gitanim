...
for my $mode (qw(uniform adaptive gaussian review)) {
    my $out = `$DIFFVIM --pacing $mode --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
    ok("--pacing $mode accepted", $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --pacing invalid tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--pacing invalid rejected', $bad =~ /invalid --pacing value/);

# Test 4: Old flags are rejected (removed)
my $out_old_adapt = `$DIFFVIM --adaptive tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--adaptive rejected', $out_old_adapt =~ /Unknown option: --adaptive/);

my $out_old_at = `$DIFFVIM --adaptive-timing tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--adaptive-timing rejected', $out_old_at =~ /Unknown option: --adaptive-timing/);

my $out_old_gj = `$DIFFVIM --gaussian-jitter tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--gaussian-jitter rejected', $out_old_gj =~ /Unknown option: --gaussian-jitter/);

my $out_old_pal = `$DIFFVIM --pause-after-lines 5 tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
...
my $out_combo = `$DIFFVIM --pacing review --delete-pacing word --insert-pacing word --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
ok('--pacing combines with --delete-pacing and --insert-pacing', $out_combo =~ /---/);

# Test 7: --pacing combines with --op-order
my $out_combo2 = `$DIFFVIM --pacing adaptive --op-order end-first-smart --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
...
my $out_combo3 = `$DIFFVIM --pacing gaussian --speed 2 --dry-run tests/examples/01_small_python/old.py tests/examples/01_small_python/new.py 2>&1`;
