...
my $out_from = run_with_timeout("$DIFFVIM --from HEAD~3 examples/01_small_python/old.py examples/01_small_python/new.py", 3);
ok('--from rejected as unknown', $out_from =~ /Unknown option: --from/);

# Test 3: --to is rejected as unknown option
my $out_to = run_with_timeout("$DIFFVIM --to HEAD examples/01_small_python/old.py examples/01_small_python/new.py", 3);
ok('--to rejected as unknown', $out_to =~ /Unknown option: --to/);

# Test 4: --auto-precompute is rejected as unknown option
my $out_ap = run_with_timeout("$DIFFVIM --auto-precompute examples/01_small_python/old.py examples/01_small_python/new.py", 3);
...
my $out_ct = run_with_timeout("$DIFFVIM --compute-tool c examples/01_small_python/old.py examples/01_small_python/new.py", 3);
...
my $out_gr = run_with_timeout("$DIFFVIM --git-rev HEAD~1..HEAD --dry-run examples/01_small_python/old.py", 5);
ok('--git-rev is accepted (not "Unknown option")', $out_gr !~ /Unknown option: --git-rev/);

# Test 7: --replay is parsed (not "Unknown option")
my $out_replay = run_with_timeout("$DIFFVIM --replay --dry-run examples/01_small_python/old.py", 5);
ok('--replay is accepted (not "Unknown option")', $out_replay !~ /Unknown option: --replay/);

# Test 8: --precomputed still works (kept)
my $out_pc = run_with_timeout("$DIFFVIM --precomputed /tmp/nonexistent.diff --dry-run examples/01_small_python/old.py examples/01_small_python/new.py", 3);
ok('--precomputed is accepted (not "Unknown option")', $out_pc !~ /Unknown option: --precomputed/);

# Test 9: --multi still works (kept)
my $out_multi = run_with_timeout("$DIFFVIM --multi --dry-run examples/01_small_python/old.py:examples/01_small_python/new.py", 3);
ok('--multi is accepted', $out_multi =~ /---/ || $out_multi !~ /Unknown option/);

# Test 10: Short options -r and -R still work
my $out_r = run_with_timeout("$DIFFVIM -r --dry-run examples/01_small_python/old.py", 5);
ok('-r is accepted (not "Unknown option")', $out_r !~ /Unknown option: -r/);

my $out_R = run_with_timeout("$DIFFVIM -R HEAD~1..HEAD --dry-run examples/01_small_python/old.py", 5);
ok('-R is accepted (not "Unknown option")', $out_R !~ /Unknown option: -R/);
