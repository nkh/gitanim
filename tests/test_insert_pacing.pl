...
for my $mode (qw(char word accel)) {
    my $out = `$DIFFVIM --insert-pacing $mode --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--insert-pacing $mode accepted", $out =~ /---/);
}

# Test 3: Invalid mode is rejected
my $bad = `$DIFFVIM --insert-pacing invalid tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
for my $speed (qw(slow normal fast)) {
    my $out = `$DIFFVIM --insert-speed $speed --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
    ok("--insert-speed $speed accepted", $out =~ /---/);
}

# Test 5: Invalid speed is rejected
my $bad_speed = `$DIFFVIM --insert-speed invalid tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--insert-speed invalid rejected', $bad_speed =~ /invalid --insert-speed value/);

# Test 6: Old flags are rejected (removed)
my $out_old_mwc = `$DIFFVIM --max-word-chars 5 tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--max-word-chars rejected', $out_old_mwc =~ /Unknown option: --max-word-chars/);

my $out_old_wpm = `$DIFFVIM --word-pause-ms 200 tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--word-pause-ms rejected', $out_old_wpm =~ /Unknown option: --word-pause-ms/);

my $out_old_wa = `$DIFFVIM --word-accel tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--word-accel rejected', $out_old_wa =~ /Unknown option: --word-accel/);

# Test 7: Env vars work
$ENV{AD_INSERT_PACING} = 'word';
my $out_env = `$DIFFVIM --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('AD_INSERT_PACING env var works', $out_env =~ /---/);
delete $ENV{AD_INSERT_PACING};

$ENV{AD_INSERT_SPEED} = 'fast';
my $out_env_sp = `$DIFFVIM --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('AD_INSERT_SPEED env var works', $out_env_sp =~ /---/);
delete $ENV{AD_INSERT_SPEED};

# Test 8: --insert-pacing combines with --delete-pacing
my $out_combo = `$DIFFVIM --insert-pacing word --delete-pacing word --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--insert-pacing combines with --delete-pacing', $out_combo =~ /---/);

# Test 9: --insert-pacing combines with --op-order
my $out_combo2 = `$DIFFVIM --insert-pacing accel --op-order left-to-right --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
ok('--insert-pacing combines with --op-order', $out_combo2 =~ /---/);

# Test 10: --insert-pacing accel + --delete-pacing accel combine
my $out_combo3 = `$DIFFVIM --insert-pacing accel --delete-pacing accel --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
...
my $out_override = `$DIFFVIM --insert-pacing word --max-word-chars 3 --dry-run tests/tests/examples/01_small_python/old.py tests/tests/examples/01_small_python/new.py 2>&1`;
