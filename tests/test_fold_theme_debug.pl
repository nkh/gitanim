...
print "\n=== Test: --fold-unchanged ===\n";
my $out = `perl diffvim.pl --fold-unchanged --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
ok('--fold-unchanged with --dry-run works', $out =~ /Dry run/);

# Test 3: --theme accepts valid values
print "\n=== Test: --theme ===\n";
for my $theme ('dark', 'light', 'high-contrast') {
    $out = `perl diffvim.pl --theme $theme --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
...
$out = `perl diffvim.pl --debug --dry-run examples/01_small_python/old.py examples/01_small_python/new.py 2>&1`;
