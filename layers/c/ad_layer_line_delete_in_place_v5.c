/* ad_layer_line_delete_in_place_v5.c — Clone with NO position recompute.
 *
 * Same LDI reordering, but does NOT recompute positions at all.
 * Just reorders ops and keeps original positions from compute.
 * The animator's clamping (line > n_lines → last line, col > len → end)
 * handles the rest.
 *
...
static int layer_line_delete_in_place_v5(Op *ops, int n_ops, Op *out, int out_cap, int *line_offset) {
    (void)line_offset;
        ...
        if (i + 2 < n_work
            ...
            if (strcmp(work[i+1].type, "delete") == 0
                ...
                if (ce < n_work
                    ...
                    /* Emit content deletes (ORIGINAL positions, no changes) */
    ...
    /* NO position recompute — keep original positions from compute */
    return n_out;
}

int main(void) {
    return ad_layer_run(layer_line_delete_in_place_v5);
}
