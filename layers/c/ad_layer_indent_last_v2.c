/* ad_layer_indent_last_v2.c — Clone of indent_last using shared position-walk.
 *
 * Same algorithm as ad_layer_indent_last.c, but uses
 * ad_layer_recompute_positions() instead of no line recompute.
 *
 * NOT added to git — experimental clone for testing.
 */
#include "ad_layer_common.h"

static int layer_indent_last_v2(Op *ops, int n_ops, Op *out, int out_cap, int *line_offset) {
    (void)line_offset;
...
int main(void) {
    return ad_layer_run(layer_indent_last_v2);
}
