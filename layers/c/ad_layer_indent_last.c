...
int main(void) {
        ...
        if(strncmp(line,"HUNK\t",5)==0){
            if(in_hunk&&n_ops>0){
                ...
                for(int i=0;i<=n_ops;i++){
                    int is_b=(i==n_ops);
                    if(i<n_ops&&i>seg_start){if(ops[i].code==10&&!ad_layer_is_debug_op(&ops[i]))is_b=1;if(!ad_layer_is_debug_op(&ops[i])&&!ad_layer_is_debug_op(&ops[i-1])){if(ops[i].line!=ops[i-1].line)is_b=1;}}
                    if(is_b){
                        int sl=i-seg_start;
                        if(sl>0){
                            int ie=seg_start;
                            for(int j=seg_start;j<i;j++){if(ad_layer_is_debug_op(&ops[j]))continue;if(strcmp(ops[j].type,"delete")==0&&(ops[j].code==32||ops[j].code==9))ie=j+1;else break;}
                            int ni=ie-seg_start;
                            if(ni==0){
                                for(int j=seg_start;j<i;j++)out[n_out++]=ops[j];
                            }else{
                                int nl=-1;
                                for(int j=i-1;j>=ie;j--){if(!ad_layer_is_debug_op(&ops[j])&&ops[j].code==10){nl=j;break;}}
                                int ce=(nl>=0)?nl:i;
                ...
                for(int i=0;i<n_out;i++)ad_layer_write_op(&out[i]);
        ...
        if(strncmp(line,"HUNK_END",8)==0){
            if(in_hunk&&n_ops>0){
                ...
                for(int i=0;i<=n_ops;i++){
                    int is_b=(i==n_ops);
                    if(i<n_ops&&i>seg_start){if(ops[i].code==10&&!ad_layer_is_debug_op(&ops[i]))is_b=1;if(!ad_layer_is_debug_op(&ops[i])&&!ad_layer_is_debug_op(&ops[i-1])){if(ops[i].line!=ops[i-1].line)is_b=1;}}
                    if(is_b){int sl=i-seg_start;if(sl>0){int ie=seg_start;for(int j=seg_start;j<i;j++){if(ad_layer_is_debug_op(&ops[j]))continue;if(strcmp(ops[j].type,"delete")==0&&(ops[j].code==32||ops[j].code==9))ie=j+1;else break;}int ni=ie-seg_start;if(ni==0){for(int j=seg_start;j<i;j++)out[n_out++]=ops[j];}else{int nl=-1;for(int j=i-1;j>=ie;j--){if(!ad_layer_is_debug_op(&ops[j])&&ops[j].code==10){nl=j;break;}}int ce=(nl>=0)?nl:i;for(int j=ie;j<ce;j++){out[n_out]=ops[j];out[n_out].col=ops[j].col+ni;n_out++;}for(int j=seg_start;j<ie;j++){out[n_out]=ops[j];out[n_out].col=1;n_out++;}if(nl>=0)out[n_out++]=ops[nl];}}seg_start=i;}
                }
                ad_layer_write_hunk(&hunk);
                for(int i=0;i<n_out;i++)ad_layer_write_op(&out[i]);
                ad_layer_write_hunk_end();
    ...
    if(in_hunk&&n_ops>0){
        Op *out=(Op *)malloc((n_ops+1024)*sizeof(Op));int n_out=0,seg_start=0;
        for(int i=0;i<=n_ops;i++){int is_b=(i==n_ops);if(i<n_ops&&i>seg_start){if(ops[i].code==10&&!ad_layer_is_debug_op(&ops[i]))is_b=1;if(!ad_layer_is_debug_op(&ops[i])&&!ad_layer_is_debug_op(&ops[i-1])){if(ops[i].line!=ops[i-1].line)is_b=1;}}if(is_b){int sl=i-seg_start;if(sl>0){int ie=seg_start;for(int j=seg_start;j<i;j++){if(ad_layer_is_debug_op(&ops[j]))continue;if(strcmp(ops[j].type,"delete")==0&&(ops[j].code==32||ops[j].code==9))ie=j+1;else break;}int ni=ie-seg_start;if(ni==0){for(int j=seg_start;j<i;j++)out[n_out++]=ops[j];}else{int nl=-1;for(int j=i-1;j>=ie;j--){if(!ad_layer_is_debug_op(&ops[j])&&ops[j].code==10){nl=j;break;}}int ce=(nl>=0)?nl:i;for(int j=ie;j<ce;j++){out[n_out]=ops[j];out[n_out].col=ops[j].col+ni;n_out++;}for(int j=seg_start;j<ie;j++){out[n_out]=ops[j];out[n_out].col=1;n_out++;}if(nl>=0)out[n_out++]=ops[nl];}}seg_start=i;}}
        ad_layer_write_hunk(&hunk);
        for(int i=0;i<n_out;i++)ad_layer_write_op(&out[i]);
        ad_layer_write_hunk_end();
        free(out);
