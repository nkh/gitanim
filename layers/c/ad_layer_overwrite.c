    ...
    while (fgets(line,sizeof(line),stdin)) {
        ...
        if(strncmp(line,"HUNK\t",5)==0){
            if(in_hunk&&n_ops>0){
                ...
                /* set positions */
                {int cl=n_out>0?out[0].line:1,cc=1;for(int i=0;i<n_out;i++){if(ad_layer_is_debug_op(&out[i]))continue;out[i].line=cl;out[i].col=cc;if(strcmp(out[i].type,"keep")==0){if(out[i].code==10){cl++;cc=1;}else cc++;}else if(strcmp(out[i].type,"insert")==0||strcmp(out[i].type,"overwrite_insert")==0){if(out[i].code==10){cl++;cc=1;}else cc++;}}}
                /* update offset */
                ad_layer_write_hunk(&hunk);for(int i=0;i<n_out;i++)ad_layer_write_op(&out[i]);ad_layer_write_hunk_end();free(out);
            }
