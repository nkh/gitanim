...
function! s:StartNextHunk() abort
    ...
    if l:hunk.is_end_insert && l:target_line > line('$')
        let s:state.move_end_l = line('$')
        let s:state.move_end_c = len(getline(line('$'))) + 1
    elseif l:hunk.is_end_delete
        let l:prev = l:target_line - 1
        if l:prev < 1 | let l:prev = 1 | endif
        if l:prev > line('$') | let l:prev = line('$') | endif
        let s:state.move_end_l = l:prev
        let s:state.move_end_c = len(getline(l:prev)) + 1
    else
        if l:target_line < 1
            let l:target_line = 1
        endif
        if l:target_line > line('$')
            let l:target_line = line('$')
        endif
        let s:state.move_end_l = l:target_line
        let s:state.move_end_c = 1
    endif
