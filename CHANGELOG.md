...
`ad_postprocess` now supports `--transform NAME[:VALUE]` (repeatable)
...
The `ad_pipeline` script starts both colorizations (old + new file) in
...
- `compute/c/` (C reference) and binary `bin/ad_compute`
- `compute/rust/` (Rust port) and binary `bin/ad_compute`
- `compute/go/` (Go port) and binary `bin/ad_compute`
- `animator/go/` entirely (animator.go, pace.go, postprocess.go) plus
  compiled binaries `bin/ad`,
  `bin/ad_layer_pace`, `bin/ad_postprocess`
- `tests/test_tool.pl` (only existed to test the `--tool` flag, which
  Phase B removes)
- Myers algorithm from `compute/cpp/ad_compute.cpp` (the
...
`animator/ad_pipeline`. With only one compute tool remaining
...
- `animator/ad_pipeline` defaults to `bin/ad_compute`
  and falls back to `compute/perl/compute_builtin.pl` when the C++
  binary is missing.
- The animator default is `ad` (C++) with `animator.pl`
  as fallback. The Go animator is gone.

`COMPUTE_TOOL` env var replaced by `AD_COMPUTE_BIN` (for advanced
override only).
...
fallback when `bin/ad_compute` is not on disk.
Verified byte-identical MD5 with the C++ tool on
`tests/tests/examples/01_small_python`.
...
- mdBook (`docs/src/*.md`), reference docs (`docs/*.md`), and animator
  docs swept for stray `myers` / `--tool` / `ad_compute-{c,rust,go}`
...
07_text_prose, 33_large_python). ad_pipeline passes 42/42.
...
Result: ad_pipeline went from 6/42 OK to 42/42 OK.
