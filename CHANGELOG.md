...
The historical multi-language parity experiment is over. The compute
tools were four byte-identical implementations (C, C++, Rust, Go) that
all produced the same output; the cost of keeping four sources in sync
outweighed the benefit. The Go animator components had a similar story.

Deleted:

- `compute/c/` (C reference) and binary `compute/bin/diffvim-compute-cpp`
- `compute/rust/` (Rust port) and binary `compute/bin/diffvim-compute-cpp`
- `compute/go/` (Go port) and binary `compute/bin/diffvim-compute-cpp`
- `animator/go/` entirely (animator.go, pace.go, postprocess.go) plus
  compiled binaries `animator/bin/diffvim-animator`,
  `animator/bin/diffvim-pace`, `animator/bin/diffvim-postprocess`
- `tests/test_tool.pl` (only existed to test the `--tool` flag, which
  Phase B removes)
- Myers algorithm from `compute/cpp/diffvim-compute.cpp` (the
  `myers_diff()` function, the `myers` branch in `compute_line_diff`,
  the usage strings, and the help text)

Survivors:
- compute: `compute/cpp/diffvim-compute.cpp` → `compute/bin/diffvim-compute-cpp`
...
The `compute/Makefile` was rewritten to be C++-only: targets `all` /
`cpp` / `benchmark` / `clean`; the `RUSTC`, `GO`, `RUSTFLAGS`,
`BIN_C`, `BIN_RUST`, `BIN_GO` variables and all C/Rust/Go rules are
gone.

### Removed — Phase B: `--tool` flag

The `--tool c|cpp|rust|go` option is gone from both `diffvim` and
`animator/diffvim-pipeline`. With only one compute tool remaining
(C++), the flag was redundant.

Default behaviour:

- `diffvim` (and `diffvim-tmux`, `diffvim.pl` via shared flag parsing)
  now look for `compute/bin/diffvim-compute-cpp` in `compute/bin/`,
  `/usr/local/bin/`, and `~/.local/bin/`. If found, the diff is
  pre-computed before launching vim (10-100x faster than the in-vim
  LCS for large files). If not found, the embedded vimscript LCS
  (`s:LineDiff` / `s:CharDiff`) is used with a warning on stderr.
- `animator/diffvim-pipeline` defaults to `compute/bin/diffvim-compute-cpp`
  and falls back to `compute/perl/compute_builtin.pl` when the C++
  binary is missing.
- The animator default is `diffvim-animator-c` (C++) with `animator.pl`
  as fallback. The Go animator is gone.

`COMPUTE_TOOL` env var replaced by `DIFFVIM_COMPUTE_BIN` (for advanced
override only).

### Added — `compute/perl/compute_builtin.pl`

A ~90-line wrapper around `DiffVim::Parser::Perl::parse_diff` that
emits the same op-stream format as the C++ binary. Used as the
fallback when `compute/bin/diffvim-compute-cpp` is not on disk.
Verified byte-identical MD5 with the C++ tool on
`examples/01_small_python`.

### Removed — `--algorithm myers`

`--algorithm` now accepts only `lcs` (default) and `patience`. The
Myers implementation used O(N×M) memory and was OOM-killed on 15K-line
files; it produced the same op count as LCS, so it added complexity
without benefit.

### Changed — Phase C: cursor positioning moved from pace to postprocess

The timed op stream was upgraded from **v1** (space-separated,
`glide <line>:<col>` ops between hunks) to **v2** (TSV, every op
carries its own 1-indexed `(line, col)`).
...
Stage responsibilities are now:

- **postprocess**: owns cursor positioning. Walks each hunk's ops,
  simulates the cursor position, emits a `(line, col)` for every op.
  Tracks `line_offset` (cumulative `newline_inserts − newline_deletes`
  from previous hunks) so each op targets the right *current* buffer
  line. Adds per-op positioning so the animator is scroll-safe.
- **pace**: owns ONLY delays and batching. No more `glide` emission,
  no more `current_line` / `current_col` / `line_offset` tracking.
- **animator**: no `glide` handler. Calls `set_cursor(line, col)`
  before applying each op. When the target line is beyond the buffer
  end (the end-insert case), the cursor goes to the END of the last
  line so subsequent inserts append correctly.

Bug fix in pace: `process_awd()` was incorrectly incrementing `cur_col`
after deletes. Deletes don't advance the column (the next char slides
into the same col). All `batch_delete` ops in a run now target the
SAME `(line, col)`. This was the root cause of example 02 producing
`piocessed` instead of `processed`.

Applied to both the C and Perl implementations of postprocess / pace /
animator. The Go versions no longer exist.

### Verified

- All 42 example pairs pass MD5 round-trip verification with the
  default C++ compute + C animator pipeline.
- With the C++ binary removed, the Perl fallback path also produces
  byte-identical output.
- Cross-language parity tests: 14/14 pass (C == Perl for postprocess
  + pace across all 4 delete-pacing modes).
- Round-trip tests: 15/15, 30/30, 30/30 pass (Perl and C animators
  produce identical output).
- `test_newline_fix`: 7/7 pass.
- 171 tests pass across the suites touched.
...
- `man/diffvim.1`, `man/diffvim-compute.1`, `man/diffvim-compare.1`
...
- mdBook (`docs/src/*.md`), reference docs (`docs/*.md`), and animator
  docs swept for stray `myers` / `--tool` / `diffvim-compute-{c,rust,go}`
  / `pace.go` / `postprocess.go` / `animator.go` / `glide` / Go
  animator references.

### Known issues — UNRESOLVED

1. **Ghost line problem** (Phase F pending): when a `\n` delete joins
   two lines, the next line's content visually jumps up. The fix
   belongs in postprocess (transform the ops), not in the animator.
   NOT YET IMPLEMENTED.

2. **Large-file performance**: vimscript engine is O(N²) on large op
   lists. 28K ops (33_large_python) takes 11+ seconds; 68K ops
   (42_large_huge_python) times out at 30s. The standalone C
   animator handles these fine (< 1 second).

3. **Pause/resume cursor drift**: in interactive diffvim, cursor
   position is not re-validated after pause/scroll/resume.

4. **Pre-existing failure**: `animator/tests/test_synchronous_engine.pl`
   (10/10 fail with "no output"). This is the vimscript engine running
   in synchronous mode; it was already failing before the refactor
   (NEXT_SESSION.md notes "32/42 pass" for vimscript synchronous
   mode). The vimscript engine was NOT touched in this refactor.
...
### Fixed — same regression in C/Perl animators
...
`set_config`, and all four `diffvim-compute-cpp` variants
support it too.
