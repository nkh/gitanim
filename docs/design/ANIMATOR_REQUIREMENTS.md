# ad: Standalone Animation Application
...
A **standalone terminal application** (`ad`) that:
...
old.py + new.py
       ...
       ▼
┌──────────────────┐
│ ad_compute   │  (existing: C++ with Perl fallback)
│                   │
│ Input: old, new   │
...
|------|---------|-------------|
| ad_compute | C++ (exists) | Perl fallback (`compute_builtin.pl`) |
| ad_postprocess | C | Perl |
| ad_layer_pace | C | Perl |
| ad | C | Perl |
...
#### FR-2: The Post-Processing Tool (ad_postprocess)
...
#### FR-3: The Pacing Tool (ad_layer_pace)
...
```
ad [options] <oldfile>
ad [options] --multi <old1:new1> <old2:new2> ...
...
```
ad_layer_pace [options]  < ordered_ops  > timed_ops
...
- Implement `ad_postprocess` in Perl (primary), C, Go
- Implement `ad_layer_pace` in Perl (primary), C, Go
