...
`ad_vim` searches for it in `bin/ad_compute `, `/usr/local/bin/`, and
...
(`s:LineDiff` / `s:CharDiff` in `autoload/ad_vim/engine.vim`) with a
...
`diff_engine/perl/compute_builtin.pl` wrapper.
