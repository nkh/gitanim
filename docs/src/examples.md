...
The repo includes example file pairs in `tests/examples/`, each in a separate
directory:
...
```bash
ad_vim --multi \
    tests/examples/01_small_python/old.py:tests/examples/01_small_python/new.py \
    tests/examples/04_shell_script/old.sh:tests/examples/04_shell_script/new.sh
```
