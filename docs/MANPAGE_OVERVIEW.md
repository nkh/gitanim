...
The `man/` directory contains 16 manpages for the `ad` toolkit. Install with `make install-man` (installs to `$(PREFIX)/share/man/man1/`).
...
| `ad_layer_highlight.1` | `bin/ad_layer_highlight` | Highlight layer (insert highlight/dim/fold ops) |
...
The following tools do NOT have manpages yet:

| Tool | Why it's missing |
|------|-----------------|
| `ad_pipeline` | The pipeline driver — should be documented. |
| `ad_postprocess` | The layer orchestrator — should be documented. |
| `ad` (animator) | The C animator binary — should be documented. |
| `ad_layer_reorder` | Internal layer — low priority. |
| `ad_layer_overwrite` | Internal layer — low priority. |
| `ad_layer_indent_last` | Internal layer — low priority. |
| `ad_layer_line_delete_in_place` | Internal layer — low priority. |
| `ad_layer_pace` | Internal layer — low priority. |
| `ad_suggest.sh` | Helper script — low priority. |
